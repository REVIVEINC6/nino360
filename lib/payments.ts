...
function loadStripe() {
  ...
  const Stripe = require("stripe").Stripe as any
  // apiVersion string may be a narrow literal in installed stripe types; cast options to any to avoid type mismatch for now.
...
export async function createCheckoutSession(input: CheckoutInput): Promise<CheckoutResult> {
  ...
  // Disallow mock provider in production flows. Require Stripe configuration.
  throw new Error(
    "Payment provider not configured. Set PAYMENT_PROVIDER=stripe and provide Stripe keys in environment variables.",
  )
}
