...
function generateACHFile(payments: Payment[], metadata?: Record<string, any>): string {
  ...
  payments.forEach((payment, index) => {
    ...
    const amountValue = Math.round(payment.amount * 100)
    const amount = amountValue.toString().padStart(10, "0")
...
export async function encryptBankData(data: string): Promise<string> {
  ...
  const dataBuffer = encoder.encode(data) // Uint8Array

  // Ensure we reference the global Web Crypto where available
  const subtle = (globalThis.crypto && (globalThis.crypto as any).subtle) || (crypto as any).subtle

  const keyMaterial = encoder.encode(process.env.ENCRYPTION_KEY || "default-key-change-me")
  // subtle.importKey accepts an ArrayBuffer/ArrayBufferView; pass the underlying buffer for clarity
  const key = await subtle.importKey("raw", keyMaterial.buffer, { name: "AES-GCM" }, false, ["encrypt"])

  const iv = (globalThis.crypto && (globalThis.crypto as any).getRandomValues)
    ? (globalThis.crypto as any).getRandomValues(new Uint8Array(12))
    : new Uint8Array(12)

  const encrypted = await subtle.encrypt({ name: "AES-GCM", iv }, key, dataBuffer.buffer)
  ...
  // Convert to base64 using Buffer when running in Node
...
export async function decryptBankData(encryptedData: string): Promise<string> {
  const encoder = new TextEncoder()
  const combinedBuf = Buffer.from(encryptedData, "base64")

  const iv = combinedBuf.subarray(0, 12)
  const encrypted = combinedBuf.subarray(12)

  const subtle = (globalThis.crypto && (globalThis.crypto as any).subtle) || (crypto as any).subtle

  const keyMaterial = encoder.encode(process.env.ENCRYPTION_KEY || "default-key-change-me")
  const key = await subtle.importKey("raw", keyMaterial.buffer, { name: "AES-GCM" }, false, ["decrypt"])

  // Pass the Buffer (which is a Uint8Array subclass) directly to subtle.decrypt
  // Ensure we pass a Uint8Array view for the iv
  const ivView = iv instanceof Uint8Array ? iv : new Uint8Array(iv)
  const decrypted = await subtle.decrypt({ name: "AES-GCM", iv: ivView }, key, encrypted)
