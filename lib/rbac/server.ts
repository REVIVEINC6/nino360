...
/**
 * Guard and return tenant id for the current user.
 * Throws if tenant_id not present or user not authorized.
 */
export async function guardTenantAccess(): Promise<string> {
  const supabase = await createServerClient()
  const { data } = await supabase.auth.getUser()
  const user = data.user
  if (!user) throw new Error('Unauthorized')
  const tenantId = user.user_metadata?.tenant_id
  if (!tenantId) throw new Error('Tenant access required')
  // Optionally, verify the user has at least read access
  // For now, rely on tenant metadata.
  return tenantId
}
