...
async function fetcher<T>(action: () => Promise<any>) {
  const result = await action()
  // Normalize various server-action shapes to the { success, data } contract
  const normalized: { success: boolean; data?: T; error?: string } =
    typeof result === "object" && result !== null && "success" in result
      ? result
      : { success: true, data: result }

  if (!normalized.success) {
    throw new Error(normalized.error || "Failed to fetch data")
  }
