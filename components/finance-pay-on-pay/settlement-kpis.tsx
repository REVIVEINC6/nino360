    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      {kpis.map((kpi, index) => {
        ...
        return (
          <Card
            key={index}
            className="relative overflow-hidden bg-linear-to-br from-background/80 to-background/40 backdrop-blur-sm border-white/10"
          >
            <CardContent className="p-6">
              <div className="flex items-start justify-between">
                ...
                <div className={`p-3 rounded-lg bg-linear-to-br ${kpi.gradient} opacity-20`}>
                  <Icon className="h-6 w-6" />
                </div>
              </div>
              <div className={`absolute bottom-0 left-0 right-0 h-1 bg-linear-to-r ${kpi.gradient}`} />
            </CardContent>
          </Card>
        )
      })}
