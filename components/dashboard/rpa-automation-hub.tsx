...
export function RpaAutomationHub() {
  ...
  const toggleAutomation = async (id: string) => {
    try {
      ...
      const res = await fetch('/api/tenant/rpa/toggle', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status: nextStatus }) })
      if (!res.ok) throw new Error('Toggle failed')
      setAutomations((prev) => prev.map((a) => a.id === id ? { ...a, status: nextStatus, progress: nextStatus === 'paused' ? 0 : a.progress } : a))
      toast({ title: nextStatus === 'paused' ? 'Automation Paused' : 'Automation Started', description: current?.name })
    } catch (e) {
      toast({ title: 'Action failed', description: 'Could not toggle automation', variant: 'destructive' as any })
    }
  }
  ...
  return (
    <Card className="glass-panel border-primary/20">
      <CardHeader className="bg-linear-to-r from-purple-500/10 to-transparent">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (!name) return
              try {
                const res = await fetch('/api/tenant/rpa/new', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) })
                if (!res.ok) throw new Error('Create failed')
                const data = await res.json()
                setAutomations((prev) => [{ id: data.id, name, description: 'New automation', status: 'paused', progress: 0, trigger: 'Manual', lastRun: 'never', executionCount: 0, successRate: 0 }, ...prev])
                toast({ title: 'Automation Created', description: name })
              } catch (e) {
                toast({ title: 'Create failed', description: 'Could not create automation', variant: 'destructive' as any })
              }
            }}
          >
