      ...
      <CardContent>
        {loading && (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-28 rounded-lg bg-gradient-to-r from-gray-200/50 to-gray-100/50 animate-pulse" />
            ))}
          </div>
        )}
          ...
          <div className="space-y-3">
            {recommendations.slice(0, 4).map((rec, index) => (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-lg backdrop-blur-sm bg-gradient-to-br from-white/80 to-white/40 border border-white/30 hover:shadow-xl transition-all"
              >
                {/* Priority indicator */}
