...
// Use API endpoint to fetch personalized insights (ensures auth and RBAC go through server)
const INSIGHTS_API = "/api/dashboard/insights"
...
export function PersonalizedInsightsPanel() {
  ...
  const loadInsights = async () => {
    ...
    try {
      try {
        const res = await fetch(INSIGHTS_API, { next: { revalidate: 30 } })
        if (!res.ok) {
        ...
        console.error("[v0] fetch insights error:", err)
      }
    } catch (err) {
      ...
      <CardContent>
        {loading && (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-24 rounded-lg bg-linear-to-r from-gray-200/50 to-gray-100/50 animate-pulse" />
            ))}
          </div>
        )}
          ...
          <div className="space-y-3">
            {insights.map((insight, index) => (
              <motion.div
                ...
                className="p-4 rounded-lg backdrop-blur-sm bg-linear-to-br from-white/80 to-white/40 border border-white/30 hover:shadow-lg transition-all"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 flex-1">
                    <div className="p-2 rounded-lg bg-linear-to-br from-blue-500/10 to-purple-500/10">
