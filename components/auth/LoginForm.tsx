...
export function LoginForm() {
  ...
  const onSubmit = async (data: LoginInput) => {
    ...
    try {
      // Call server-side login API which will set a secure cookie on success
      const res = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: data.email, password: data.password, userAgent: navigator.userAgent }),
      })
      ...
      // Redirect to returnTo URL or dashboard by default; server has set cookie so middleware should detect auth
  ...
  const showGoogleButton = process.env.NEXT_PUBLIC_SUPABASE_GOOGLE === "true"
