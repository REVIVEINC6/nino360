  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="p-6 bg-linear-to-br from-pink-50 to-blue-50 border-pink-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Shared</p>
              <p className="text-3xl font-bold bg-linear-to-r from-pink-600 to-blue-600 bg-clip-text text-transparent">
                {data.sharedReports}
              </p>
            </div>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {data.recentReports.map((report: any) => (
              <div
                key={report.id}
                className="flex items-center justify-between p-4 rounded-lg border bg-linear-to-r from-blue-50/50 to-purple-50/50"
              >
        ...
        <Card className="p-6">
          ...
          <div className="space-y-3">
            {data.popularReports.map((report: any, index: number) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 rounded-lg bg-linear-to-r from-purple-50/50 to-pink-50/50"
              >
                <span className="font-medium">{report.name}</span>
                <Badge variant="secondary" className="bg-linear-to-r from-purple-100 to-pink-100">
                  {report.runs} runs
                </Badge>
              </div>
            ))}
