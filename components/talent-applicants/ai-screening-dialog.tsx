...
export function AIScreeningDialog({ open, onOpenChange, applicantIds, onScreeningComplete }: AIScreeningDialogProps) {
  ...
  const handleScreen = async () => {
    ...
    setScreening(true)
    try {
      const response: any = await runAIScreening(applicantIds, templateId)

      // The server action may return different shapes (e.g. { results: [...] })
      // or a normalized { success, data, error } shape. Handle both safely.
      if (response && Array.isArray(response.results)) {
        ...
        toast({
          ...
          description: (response && response.error) || "Failed to run AI screening",
          variant: "destructive",
        })
      }
    } catch (error) {
