...
export function ResumeParserDialog({ open, onOpenChange, applicantId, onParseComplete }: ResumeParserDialogProps) {
  ...
  const handleParse = async () => {
    ...
    setParsing(true)
    try {
      // The server action expects a File (or separate fields). Pass the File
      // directly and handle the returned shape ({ parsed } or { success, data }).
      const response: any = await parseResume(file)

      if (response && response.parsed) {
        setResult(response.parsed)
        toast({
        ...
        toast({
          ...
          description: (response && response.error) || "Failed to parse resume",
          variant: "destructive",
        })
      }
    } catch (error) {
