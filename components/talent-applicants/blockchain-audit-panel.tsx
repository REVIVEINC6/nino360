...
export function BlockchainAuditPanel({ applicantId, auditRecords }: BlockchainAuditPanelProps) {
  ...
  const handleVerify = async (recordId: string) => {
    setVerifying(true)
    try {
      const response: any = await verifyBlockchainAudit(recordId)

      // The action returns a shape like { valid, hash, transaction_id, verified_at }
      if (response && (response.valid !== undefined || response.verified_at)) {
        const verified = Boolean(response.valid)
        toast({
          title: verified ? "Verification successful" : "Verification failed",
          description: verified ? "Audit record is valid" : "Verification failed",
        ...
        toast({
          ...
          description: (response && response.error) || "Failed to verify audit record",
          variant: "destructive",
        })
      }
    } catch (error) {
