...
type RechartsPayload = any[] | undefined
...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, any>((props, ref) => {
  ...
  const tooltipLabel = React.useMemo(() => {
    if (hideLabel || !payload || (payload as any[]).length === 0) return null
      ...
      >
        ...
        <div className="grid gap-1.5">
          {((payload as any[]) || []).map((item: any, index: number) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
            // item.payload may be unknown; guard access
            const itemPayloadAny: any = item.payload
            const indicatorColor = color || itemPayloadAny?.fill || item.color

            return (
              <div
                key={(item.dataKey as any) ?? index}
                className={cn(
...
const ChartLegendContent = React.forwardRef<HTMLDivElement, any>((props, ref) => {
  ...
  if (!payload || (payload as any[]).length === 0) return null
