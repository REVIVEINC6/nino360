  ...
  return (
    <>
      <Card className="group relative overflow-hidden border-white/10 bg-white/5 p-6 backdrop-blur-xl transition-all hover:border-[#8B5CF6]/50 hover:shadow-[0_0_20px_rgba(139,92,246,0.3)]">
        {/* Neon border effect */}
        <div className="absolute inset-0 rounded-lg bg-linear-to-r from-[#4F46E5] via-[#8B5CF6] to-[#A855F7] opacity-0 blur-xl transition-opacity group-hover:opacity-20" />
            ...
            <div className="flex flex-wrap gap-2">
              {tenant.features.map((feature) => (
                <Badge
                  key={feature}
                  variant="outline"
                  className="border-white/10 bg-linear-to-r from-[#6D28D9]/20 to-[#8B5CF6]/20 text-xs"
                >
                  {feature}
                </Badge>
              ))}
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {isInvited ? (
              <>
                <Button
                  onClick={onAccept}
                  className="flex-1 bg-linear-to-r from-[#4F46E5] to-[#8B5CF6] hover:from-[#4338CA] hover:to-[#7C3AED]"
                >
                  Accept
                </Button>
              ...
              <>
                ...
                {/* Advanced actions: AI summary, Export, RPA (only for admins/managers) */}
                {(quickLinks.admin || quickLinks.settings) && (
                  <>
                    <Button
                      variant="outline"
                      size="icon"
                      ...
                      onClick={async () => {
                        try {
                          const p = await fetch(`/api/tenant/permissions?tenant_id=${tenant.id}`)
                          const pj = await p.json()
                          if (!pj.ok) return toast.error('Permission check failed')
                          const roles = pj.permissions.roles || []
                          const roleKeys = roles.map((r: any) => r.key)
                          if (!roleKeys.includes('admin') && !roleKeys.includes('manager')) return toast.error('Admin/Manager role required')

                          toast('Generating AI summary...')
                          const res = await fetch('/api/tenant/configuration/generate', {
                            method: 'POST',
                            body: JSON.stringify({ prompt: `Summarize tenant ${tenant.name}`, tenant_id: tenant.id }),
                          })
                          const j = await res.json()
                          if (j.ok) toast.success('AI suggestion applied (preview)')
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const roles = pj.permissions.roles || []
                          const roleKeys = roles.map((r: any) => r.key)
                          if (!roleKeys.includes('admin')) return toast.error('Admin role required')

                          toast('Exporting settings to chain...')
                          const res = await fetch('/api/tenant/configuration/export', { method: 'POST', body: JSON.stringify({ tenant_id: tenant.id }) })
                          const j = await res.json()
                          if (j.ok) toast.success('Export tx: ' + (j.data?.tx || 'ok'))
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          toast('Triggering RPA job...')
                          const res = await fetch('/api/tenant/configuration/rpa', { method: 'POST', body: JSON.stringify({ tenant_id: tenant.id, jobName: 'syncTenant', payload: {} }) })
                          const j = await res.json()
                          if (j.ok) toast.success('RPA queued')
                          else toast.error('RPA failed')
                        } catch (err) {
                          toast.error('RPA failed')
                        }
                      }}
                    >
                      RPA
