...
export function TenantDataContent() {
  ...
  const handleExport = async (format: "json" | "csv") => {
    setIsExporting(true)
    try {
      const result: any = await exportData(format)
      toast({
        title: "Success",
        description: result?.message || `Exported ${result?.count ?? 0} items`,
      })
    } catch (error) {
      toast({
        title: "Error",
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsImporting(true)
    try {
      const result: any = await importData(file)
      toast({
        title: "Success",
        description: result?.message || `Imported ${result?.count ?? 0} rows`,
      })
    } catch (error) {
      toast({
        title: "Error",
        ...
        <Card className="p-6 bg-white/50 backdrop-blur-sm border-white/20">
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Card className="p-6 bg-linear-to-br from-blue-50 to-purple-50 border-white/20">
              ...
              <Card className="p-6 bg-linear-to-br from-purple-50 to-pink-50 border-white/20">
      ...
      <TabsContent value="retention" className="space-y-6">
        <Card className="p-6 bg-white/50 backdrop-blur-sm border-white/20">
          <div className="space-y-6">
            ...
            <Button className="bg-linear-to-r from-blue-600 to-purple-600">Save Retention Policies</Button>
          </div>
        </Card>
      </TabsContent>
