      ...
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button className="p-2 text-xs bg-purple-50 text-purple-700 rounded-lg hover:bg-purple-100 transition-colors" onClick={async () => {
              try {
                const res = await fetch('/api/crm/dashboard/rpa', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ module: 'crm', event: 'manual.assess_pipeline', entity: 'opportunity', record: {} }),
                })
                const json = await res.json()
                if (json.success) toast.success(`RPA executed ${json.rulesExecuted || 0} rules`)
                else toast.error('RPA execution failed')
              } catch (err: any) {
                toast.error(err.message || 'RPA request failed')
              }
            }}>
