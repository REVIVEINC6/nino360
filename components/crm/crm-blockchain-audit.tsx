...
export function CRMBlockchainAudit({ getAuditTrail }: CRMBlockchainAuditProps) {
  ...
  if (loading || isPending) {
    return (
      <Card className="border-emerald-500/20 bg-linear-to-br from-emerald-500/5 to-teal-500/5">
        <CardHeader>
    ...
    <Card className="border-emerald-500/20 bg-linear-to-br from-emerald-500/5 to-teal-500/5">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">{auditData?.lastSync || "Never"}</span>
        </div>
        {auditData?.latestHash && (
          <div className="pt-2 space-y-2">
            ...
            <code className="block text-xs bg-muted p-2 rounded font-mono break-all">
              {auditData.latestHash.substring(0, 64)}...
            </code>
            <Link href="/settings/security/audit-log">
