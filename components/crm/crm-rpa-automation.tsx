...
export function CRMRPAAutomation({ getWorkflows }: CRMRPAAutomationProps) {
  ...
  const activeWorkflows = workflows?.workflows?.filter((w: any) => w.status === "active") || []
  const pausedWorkflows = workflows?.workflows?.filter((w: any) => w.status === "paused") || []
    ...
    <Card className="border-amber-500/20 bg-linear-to-br from-amber-500/5 to-orange-500/5">
      ...
      <CardContent className="space-y-3">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">{workflows?.emailFollowups || "0 emails sent today"}</p>
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
          ...
          <p className="text-xs text-muted-foreground">{workflows?.leadEnrichment || "0 leads enriched"}</p>
        </div>
