...
interface Message {
  ...
  role: "user" | "assistant"
  content: string
  // timestamps may be strings coming from the server; accept both
  timestamp: Date | string
  // ensure actions defaults to an array when undefined
  actions?: Array<{
    type: string
    label: string
    data?: any
...
export function AIAssistantChat() {
  ...
  const loadHistory = async () => {
    try {
      ...
      if (history.length > 0) {
        // normalize timestamps and actions
        const normalized = history.map((m: any) => ({
          ...m,
          timestamp: m.timestamp ? new Date(m.timestamp) : new Date(),
          actions: Array.isArray(m.actions) ? m.actions.map((a: any) => ({ type: a.type, label: a.label, data: a.data ?? null })) : [],
        }))
    ...
    try {
      ...
      const assistantMessage: Message = {
        ...
        actions: Array.isArray(response.actions)
          ? response.actions.map((a: any) => ({ type: a.type, label: a.label, data: a.data ?? null }))
          : [],
      }
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2">
        <Card className="glass-card border-white/20 overflow-hidden">
          <div className="p-6 border-b border-white/20 bg-linear-to-r from-indigo-500/10 via-purple-500/10 to-pink-500/10">
            <div className="flex items-center gap-3">
              <div className="p-3 rounded-xl bg-linear-to-br from-indigo-500 to-purple-500">
          ...
          <ScrollArea className="h-[500px] p-6" ref={scrollRef}>
            <AnimatePresence>
              {messages.length === 0 ? (
                ...
                >
                  <div className="p-6 rounded-full bg-linear-to-br from-indigo-500/20 to-purple-500/20 mb-4">
                  ...
                  {messages.map((message, index) => (
                    ...
                    >
                      {message.role === "assistant" && (
                        <div className="p-2 rounded-lg bg-linear-to-br from-indigo-500 to-purple-500 h-fit">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <span className="text-xs opacity-70">{new Date(typeof message.timestamp === "string" ? message.timestamp : message.timestamp || Date.now()).toLocaleTimeString()}</span>
                          {message.role === "assistant" && (
                            <>
                              <Button
                                variant="ghost"
                      ...
                      {message.role === "user" && (
                        <div className="p-2 rounded-lg bg-linear-to-br from-gray-200 to-gray-300 h-fit">
      ...
      >
        ...
        <Card className="glass-card border-white/20 p-6">
          <h3 className="text-lg font-semibold mb-4 bg-linear-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
            Quick Actions
          </h3>
