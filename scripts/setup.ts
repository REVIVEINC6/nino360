...
const SCRIPT_ORDER = [
  ...
  "32-public-rpc-wrappers.sql",
  "33-onboarding-wizard.sql",
  "34-analytics-views.sql",
]
...
async function verifySetup() {
  ...
  const { data: schemas, error: schemasError } = await supabase.rpc("exec_sql", {
    sql_query: `SELECT schema_name FROM information_schema.schemata WHERE schema_name IN ('core', 'sec', 'ai', 'app', 'ats', 'finance', 'bench', 'vms', 'hrms', 'crm', 'bill') ORDER BY schema_name`,
  })
  ...
  const { data: tables, error: tablesError } = await supabase.rpc("exec_sql", {
    sql_query: `SELECT table_schema, COUNT(*) as count FROM information_schema.tables WHERE table_schema IN ('core', 'sec', 'ai', 'app', 'ats', 'finance', 'bench', 'vms', 'hrms', 'crm', 'bill') GROUP BY table_schema ORDER BY table_schema`,
  })
  ...
  const { data: functions, error: functionsError } = await supabase.rpc("exec_sql", {
    sql_query: `SELECT routine_schema, routine_name FROM information_schema.routines WHERE routine_schema IN ('sec', 'public') AND routine_name IN ('has_permission', 'has_feature', 'get_user_permissions', 'get_user_roles') ORDER BY routine_schema, routine_name`,
  })
