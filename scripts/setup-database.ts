...
async function main() {
  ...
  // Support both forms: `--script=FILE` and `--script FILE`
  let specificScript: string | undefined = args.find((arg) => arg.startsWith("--script="))?.split("=")[1]
  if (!specificScript) {
    const idx = args.indexOf("--script")
    if (idx >= 0 && args[idx + 1]) {
      specificScript = args[idx + 1]
    }
  }
  ...
  if (specificScript) {
    // Run specific script
    console.log(`\n🎯 Running single script: ${specificScript}`)
