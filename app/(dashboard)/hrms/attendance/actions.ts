...
export async function getAttendance(input: z.infer<typeof getAttendanceSchema>) {
  try {
    ...
    const kpis = {
      present: kpisData?.filter((d) => d.status === "PRESENT").length || 0,
      absent: kpisData?.filter((d) => d.status === "ABSENT").length || 0,
      leave: kpisData?.filter((d) => d.status === "LEAVE").length || 0,
      wfh: kpisData?.filter((d) => d.status === "WFH" || d.status === "REMOTE").length || 0,
      exceptions: kpisData?.filter((d) => d.exception).length || 0,
    }
