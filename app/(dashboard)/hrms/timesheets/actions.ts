...
export async function listTimesheets() {
  ...
  const _employee: any = (employee as any) || null
  ...
  .eq("tenant_id", _employee.tenant_id)
...
export async function getMyTimesheet(input: { weekStart: string }) {
  ...
  if (!_employee) return { error: "Employee record not found" }
  ...
  // normalize timesheet for property access
  const _timesheet: any = (timesheet as any) || {}

  // Get entries
  const { data: entries } = await supabase
    .from("hr.timesheet_entries")
    .select("*, assignment:hr.assignments(id, role_title, client_id, project_id)")
    .eq("timesheet_id", _timesheet.id)
    .order("date")
...
export async function saveEntries(input: z.infer<typeof SaveEntriesSchema>) {
  ...
  const { data: timesheet } = await supabase
    ...
    .single()
  const _timesheet: any = (timesheet as any) || null

  if (!_timesheet) return { error: "Timesheet not found" }
  if (_timesheet.employee.user_id !== user.id) return { error: "Unauthorized" }
  if (_timesheet.status === "LOCKED") return { error: "Timesheet is locked" }
  ...
  // Audit log
  await appendAudit({
    tenantId: _timesheet.employee.tenant_id,
    actorUserId: user.id,
...
export async function submitTimesheet(input: z.infer<typeof SubmitTimesheetSchema>) {
  ...
  if (!_timesheet) return { error: "Timesheet not found" }
  if (_timesheet.employee.user_id !== user.id) return { error: "Unauthorized" }
  if (_timesheet.status !== "DRAFT") return { error: "Timesheet already submitted" }

  // Validate policy
  const { data: validation } = await supabase.rpc("hr.fn_policy_validate", {
    p_tenant_id: _timesheet.employee.tenant_id,
    p_employee_id: _timesheet.employee_id,
    p_week_start: _timesheet.week_start,
  })
  ...
  await supabase.from("hr.approvals").insert({
    tenant_id: _timesheet.employee.tenant_id,
    object_type: "timesheet",
    object_id: parsed.data.timesheetId,
    status: "PENDING",
    approver_id: _timesheet.employee.manager_id,
  })
...
export async function retractTimesheet(input: { timesheetId: string }) {
  ...
  if (!_timesheet) return { error: "Timesheet not found" }
  if (_timesheet.employee.user_id !== user.id) return { error: "Unauthorized" }
  if (_timesheet.status !== "PENDING") return { error: "Can only retract pending timesheets" }
...
export async function approveTimesheet(input: z.infer<typeof ApproveTimesheetSchema>) {
  ...
  if (!_timesheet) return { error: "Timesheet not found" }
  if (_timesheet.status !== "PENDING") return { error: "Timesheet not pending approval" }
  ...
  if (!approverEmployee || approverEmployee.id !== _timesheet.employee.manager_id) {
...
export async function getCompliance(input: z.infer<typeof GetComplianceSchema>) {
  ...
  const totals = (weekly || []).reduce(
    (acc: { onTime: number; late: number; missing: number }, week: any) => ({
...
export async function exportTimesheetsCsv(input: {
  ...
  if (input.scope === "employee") {
    ...
    for (const ts of timesheets || []) {
      const _ts: any = ts as any
      rows.push([
        _ts.employee?.employee_no || "",
        `${_ts.employee?.first_name || ""} ${_ts.employee?.last_name || ""}`,
        _ts.week_start,
        _ts.week_end,
        _ts.total_hours?.toString() || "0",
        _ts.billable_hours?.toString() || "0",
        _ts.status,
      ])
    }
  } else {
    ...
    for (const ts of timesheets || []) {
      const _ts: any = ts as any
      for (const entry of _ts.entries || []) {
        const _entry: any = entry as any
        rows.push([
          _ts.employee?.employee_no || "",
          `${_ts.employee?.first_name || ""} ${_ts.employee?.last_name || ""}`,
          _entry.date,
          _entry.assignment?.role_title || "Unassigned",
          _entry.task_code || "",
          _entry.hours?.toString() || "0",
          _entry.billable ? "Yes" : "No",
          _entry.notes || "",
        ])
      }
    }
