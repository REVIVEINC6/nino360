...
export async function getImmigrationCases(filters?: {
  ...
  // Supabase may return ParserError[]; cast via unknown first to silence erroneous direct conversions
  return { data: (data as unknown) as ImmigrationCase[] }
}
...
export async function getI9Records(filters?: { status?: string; reverify_due?: boolean }) {
  ...
  return { data: (data as unknown) as I9Record[] }
}
...
export async function getImmigrationAlerts(filters?: { resolved?: boolean; severity?: string }) {
  ...
  return { data: (data as unknown) as ImmigrationAlert[] }
}
