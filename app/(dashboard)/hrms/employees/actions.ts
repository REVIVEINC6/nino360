...
export async function updateProfile(input: { id: string; patch: z.infer<typeof employeeProfileSchema> }) {
  try {
    ...
    // If this is a Zod validation error, return its details
    if (error && typeof error === "object" && "errors" in error) {
      return { success: false, error: "Validation failed", details: (error as any).errors }
    }
    // safe extraction for unknown errors
    const { extractErrorDetails } = await import("@/lib/errors/extractErrorDetails")
