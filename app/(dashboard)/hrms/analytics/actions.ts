...
export async function getOverview(input: z.infer<typeof filterSchema>) {
  ...
  const totalHeadcount = headcountData?.reduce((sum, row) => sum + (row.active_count || 0), 0) || 0
  const totalJoiners = headcountData?.reduce((sum, row) => sum + (row.joiners || 0), 0) || 0
  const totalLeavers = headcountData?.reduce((sum, row) => sum + (row.leavers || 0), 0) || 0
  ...
  const totalOnTime = timesheetData?.reduce((sum, row) => sum + (row.on_time || 0), 0) || 0
  const totalLate = timesheetData?.reduce((sum, row) => sum + (row.late || 0), 0) || 0
  const totalMissing = timesheetData?.reduce((sum, row) => sum + (row.missing || 0), 0) || 0
  ...
  const totalI9Due = complianceData?.reduce((sum, row) => sum + (row.i9_due || 0), 0) || 0
  const totalI9Complete = complianceData?.reduce((sum, row) => sum + (row.i9_complete || 0), 0) || 0
  const i9OnTimePct = totalI9Due > 0 ? ((totalI9Complete / totalI9Due) * 100).toFixed(2) : "100.00"

  const totalPolicyAssigned = complianceData?.reduce((sum, row) => sum + (row.policy_assigned || 0), 0) || 0
  const totalPolicyAcked = complianceData?.reduce((sum, row) => sum + (row.policy_acked || 0), 0) || 0
  ...
  const totalOpened = casesData?.reduce((sum, row) => sum + (row.opened || 0), 0) || 0
  const totalBreached = casesData?.reduce((sum, row) => sum + (row.breached || 0), 0) || 0
