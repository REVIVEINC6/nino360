...
export async function getOnboardingOverview() {
  try {
    ...
    type OnboardingRow = { status?: string; completed_at?: string | null; created_at?: string }

    const completed = (recentOnboarding || []).filter((o: OnboardingRow) => o.status === "COMPLETED").length
    const total = (recentOnboarding || []).length || 1
    const completionRate = Math.round((completed / total) * 100)

    // Calculate average days to complete
    const completedOnboarding = (recentOnboarding || []).filter(
      (o: OnboardingRow) => o.status === "COMPLETED" && !!o.completed_at && !!o.created_at,
    )
    const avgDays =
      completedOnboarding.length > 0
        ? Math.round(
            completedOnboarding.reduce((sum: number, o: OnboardingRow) => {
              const start = new Date(o.created_at as string)
              const end = new Date(o.completed_at as string)
