...
// Import CSV
export async function importCsv(input: FormData | File | { file: string; poolId?: string }) {
  try {
    const supabase = await createClient()
    const {
      data: { user },
    ...
    // Normalize inputs: support FormData, File, or base64 payload
    ...
    if (typeof (input as any)?.get === "function") {
      ...
      poolId = (fd.get("poolId") as string) || null
      ...
      poolId = (input as any).poolId || null
    }

    if (!file && !buffer) throw new Error("File is required")

    const text = file ? await file.text() : buffer!.toString("utf8")
    ...
    const effectivePoolId = poolId || null
    if (effectivePoolId && inserted) {
    ...
    // Log import (provide fallbacks when file metadata isn't available)
    const filename = file?.name || "upload.csv"
    const sizeBytes = file?.size ?? (buffer ? buffer.length : 0)
...
export async function importZip(input: FormData | File | { file: string; poolId?: string }) {
  try {
    const supabase = await createClient()
    const {
      data: { user },
    ...
    // TODO: Implement ZIP extraction and resume parsing properly.
    // For now, keep behavior and metadata logging consistent for different input types.
