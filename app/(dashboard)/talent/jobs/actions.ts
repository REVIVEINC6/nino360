...
async function getTenantContext() {
  ...
  // Dev bypass: allow flows without a real auth session so UI remains interactive in development.
  const devBypass = process.env.ADMIN_BYPASS === "1" || process.env.DEV_BYPASS === "1"
  const ck = cookies()
  const cookieTenantId = ck.get("tenant_id")?.value || null
  if (!user && devBypass) {
    return {
      user: { id: "00000000-0000-0000-0000-000000000111" } as any,
      tenantId: (cookieTenantId as any) || "00000000-0000-0000-0000-000000000001",
      supabase,
    }
  }
  // If there's no authenticated user, return a nullable context rather than throwing.
  // Callers can decide how to proceed (readers may return empty results; writers should reject).
  if (!user) {
    return { user: null as any, tenantId: null as any, supabase }
  }

  const { data: userTenant } = await supabase.from("user_tenants").select("tenant_id").eq("user_id", user.id).single()

  if (!userTenant?.tenant_id) {
    return { user: user as any, tenantId: null as any, supabase }
  }
  ...
  try {
    ...
    if (!tenantId) {
      return {
        tenantId: null,
        tz: "UTC",
        features: { copilot: false, audit: true, publish: false },
        defaultSla: { reviewHours: 48 },
        targets: [],
      }
    }
...
export async function listRequisitions(params: z.infer<typeof listParamsSchema>) {
  try {
    ...
    // If there's no authenticated tenant or user, return an empty result instead of throwing.
    if (!tenantId || !user) {
      return { rows: [], total: 0 }
    }
    ...
    let query = supabase
      ...
      .select(
        "id, title, department, location, employment_type, seniority, openings, status, created_at, hiring_manager, recruiter_id, ai_score, ml_fill_probability, blockchain_hash",
        { count: "exact" },
      )
      .eq("tenant_id", tenantId)
...
export async function createRequisition(input: z.infer<typeof requisitionSchema>) {
  try {
    ...
    // Append blockchain-style hash to the requisition (first link)
    const rec = { tenant_id: tenantId, id: data.id, title: validated.title, created_at: data.created_at }
...
export async function updateRequisition(id: string, patch: Partial<z.infer<typeof requisitionSchema>>) {
  try {
    ...
    // Chain hash: link to previous hash
    const prevHash = original.blockchain_hash || null
    const rec = { tenant_id: tenantId, id, patch, ts: new Date().toISOString(), prev_hash: prevHash }
    const newHash = crypto.createHash("sha256").update(JSON.stringify(rec)).digest("hex")
    await supabase
      .from("talent.requisitions")
      .update({ prev_hash: prevHash, blockchain_hash: newHash })
      .eq("id", id)
      .eq("tenant_id", tenantId)
...
// ============================================================================
// AI DIGEST AND RPA MATCHING (ENHANCEMENTS)
// ============================================================================

export async function aiAssistantDigestForJobs() {
  try {
    const { tenantId, supabase } = await getTenantContext()
    const prompt = `Provide a brief, 3-bullet digest of hiring priorities and risks for this tenant. Focus on requisitions that are aging, skills gaps, and likely fill probability.`
    let text = "Enable Copilot to view AI digest."
    try {
      const res = await generateText({ model: "openai/gpt-4o-mini", prompt, maxTokens: 240 })
      text = res.text?.trim() || text
    } catch {}
    if (tenantId) {
      await supabase
        .from("app.audit_log")
        .insert({ tenant_id: tenantId, action: "copilot.query", entity: "talent.jobs", metadata: { kind: "digest" } as any })
    }
    return { text }
  } catch (e) {
    return { text: "AI unavailable right now." }
  }
}

export async function runMatching(requisition_id: string) {
  try {
    const { tenantId, user, supabase } = await getTenantContext()
    if (!tenantId || !user) throw new Error("Unauthorized")
    await supabase
      .from("app.module_usage")
      .upsert({ tenant_id: tenantId, module: "ATS", events: 1 }, { onConflict: "tenant_id,module" })
    await supabase
      .rpc("exec_sql", { sql_query: `update app.module_usage set events = events + 1, updated_at = now() where tenant_id = '${tenantId}' and module = 'ATS'` })
    await supabase
      .from("app.audit_log")
      .insert({ tenant_id: tenantId, user_id: user.id, action: "rpa.match", entity: "talent.requisition", entity_id: requisition_id })
    return { success: true }
  } catch (e) {
    console.error("[v0] runMatching error:", e)
    return { success: false }
  }
}

// ============================================================================
// PRE-CREATION AI HELPERS (for New Job page)
// ============================================================================

export async function aiComposeFromDraft(input: {
  title: string
  description?: string
  skills?: string[]
  ...
  try {
    const { tenantId, supabase } = await getTenantContext()
    const tone = input.tone || "inclusive"
    const prompt = `Compose a concise, clear job description in a ${tone} tone for the following job.
Title: ${input.title}
Existing Notes: ${input.description || "(none)"}
Skills (optional): ${(input.skills || []).join(", ")}

Return only the markdown description.`
    const { text } = await generateText({ model: "openai/gpt-4o-mini", prompt, maxTokens: 1200 })
    if (tenantId) {
      await supabase.from("app.audit_log").insert({ tenant_id: tenantId, action: "copilot.compose", entity: "talent.jobs" })
    }
    return { text }
  } catch (e) {
    return { text: "" }
...
export async function aiExtractSkillsFromDraft(input: { title: string; description?: string }) {
  try {
    const { tenantId, supabase } = await getTenantContext()
    const prompt = `Extract up to 20 key skills from the following job details. Return JSON array of strings.
Title: ${input.title}
Description: ${input.description || "(none)"}`
    const { text } = await generateText({ model: "openai/gpt-4o-mini", prompt, maxTokens: 600 })
    const skills = (() => {
      try {
    ...
    if (tenantId) {
      await supabase.from("app.audit_log").insert({ tenant_id: tenantId, action: "copilot.extract_skills", entity: "talent.jobs" })
    }
    return { skills }
  } catch (e) {
    return { skills: [] }
...
export async function aiDiversityReviewDraft(input: { description: string }) {
  try {
    const { tenantId, supabase } = await getTenantContext()
    const prompt = `Analyze this job description for inclusivity. Return JSON {issues:[{line,reason}], suggestions:[{original,replacement,reason}]}.
JD:\n${input.description}`
    const { text } = await generateText({ model: "openai/gpt-4o-mini", prompt, maxTokens: 1200 })
    let analysis: any = { issues: [], suggestions: [] }
    try {
      analysis = JSON.parse(text.trim())
    } catch {}
    if (tenantId) {
      await supabase.from("app.audit_log").insert({ tenant_id: tenantId, action: "copilot.diversity", entity: "talent.jobs" })
    }
    return analysis
  } catch (e) {
...
export async function createAndPublishRequisition(
  input: z.infer<typeof requisitionSchema>,
  targets: string[],
) {
  // Create draft then publish to selected targets
  const id = await createRequisition(input)
  const res = await publishRequisition(id, targets)
  return { id, publish: res }
}
