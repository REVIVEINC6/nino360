...
export async function getHiringForecast(tenantId: string) {
  ...
  const monthlyHires = historicalData?.reduce((acc: any, hire) => {
    const month = new Date(hire.hired_at).toISOString().slice(0, 7)
    acc[month] = (acc[month] || 0) + 1
    return acc
  }, {})

  const avgMonthlyHires =
    Object.values(monthlyHires || {}).reduce((a: any, b: any) => a + b, 0) / Object.keys(monthlyHires || {}).length
