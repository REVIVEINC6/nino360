    ...
    for (let hour = 9; hour < 17; hour++) {
      ...
      for (const panelUserId of body.panel_user_ids) {
        const hasConflict = existingInterviews?.some((interview) => {
          ...
          const isInPanel = panelMembers.some((p) => p.user_id === panelUserId)
...
export async function submitFeedback(input: unknown) {
  ...
  const dimMap = new Map(dimensions?.map((d) => [d.key, d.weight || 1]) || [])
  let totalScore = 0
  let totalWeight = 0

  for (const rating of body.ratings) {
    const weight = dimMap.get(rating.dimension_key) || 1
...
export async function getCalibrationMetrics(filters?: { job_id?: string; period?: string }) {
  ...
  const scores = aggregates.map((a) => a.avg_score)
  const mean = scores.reduce((sum, s) => sum + s, 0) / scores.length
  const variance = scores.reduce((sum, s) => sum + Math.pow(s - mean, 2), 0) / scores.length
  const sd = Math.sqrt(variance)

  // Per-interviewer stats
  const per_interviewer = aggregates.map((a) => ({
    id: a.interviewer_id,
    avg: a.avg_score,
    sd: sd, // simplified
    n: a.submissions,
  ...
  const bias_table = aggregates.map((a) => ({
...
export async function verifyHash(hash: string) {
  ...
  const { data } = await supabase.from("audit_logs").select("*").eq("hash", hash).single()
