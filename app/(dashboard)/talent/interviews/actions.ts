...
export async function getCalibrationMetrics(filters?: { job_id?: string; period?: string }) {
  ...
  try {
    ...
    if (isProfileOrAuthError(e)) {
      console.warn("[v0] Calibration: missing profile or unauthorized; returning defaults")
...
export async function listInterviews(filters?: {
  ...
  try {
    ...
    if (isProfileOrAuthError(e)) {
      console.warn("[v0] Interviews: missing profile or unauthorized; returning empty list")
      return []
    }
    throw e
  }
...
export async function getAuditMini(limit = 8) {
  ...
  try {
    ...
    if (isProfileOrAuthError(e)) {
      console.warn("[v0] AuditMini: missing profile or unauthorized; returning empty list")
      return []
    }
    throw e
  }
