...
export async function upsertTemplate(input: z.infer<typeof TemplateSchema>) {
  ...
  // matchAll returns an iterator of RegExpMatchArray; guard and type the matches
  const matches = Array.from(validated.body_md.matchAll(variableRegex) as Iterable<RegExpMatchArray>)
  const variables = matches.map((m) => (m && m[1] ? m[1].trim() : "")).filter((v) => v.length > 0)
