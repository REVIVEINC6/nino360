      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {offer.status === "draft" && (
              <Button className="bg-linear-to-r from-purple-600 to-pink-600 text-white hover:from-purple-700 hover:to-pink-700">
                <CheckCircle2 className="mr-2 h-4 w-4" />
                Submit for Approval
              </Button>
            )}
            {offer.status === "approved" && (
              <Button className="bg-linear-to-r from-blue-600 to-cyan-600 text-white hover:from-blue-700 hover:to-cyan-700">
        ...
        <Tabs defaultValue="editor" className="space-y-6">
          ...
          <TabsContent value="editor" className="space-y-6">
            <OfferEditor
              offer={offer}
              onSave={async (data) => {
                // call server action to update offer (fire-and-forget)
                try {
                  await fetch(`/talent/offers/${offer.id}/update`, {
                    method: "POST",
              ...
              onGeneratePdf={async () => {
                try {
                  await fetch(`/talent/offers/${offer.id}/pdf`, { method: "POST" })
                } catch (e) {}
              }}
              onAiDraft={async () => {
                // noop for now - server-side AI drafts not implemented here
              }}
            />
          </TabsContent>
          ...
          <TabsContent value="approvals" className="space-y-6">
            <Card className="bg-slate-900/50 backdrop-blur-sm border-purple-500/20">
              ...
              <CardContent>
                <ApprovalTimeline approvals={offer.approvals || []} />
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="versions" className="space-y-6">
            <Card className="bg-slate-900/50 backdrop-blur-sm border-purple-500/20">
              ...
              <CardContent>
                <VersionTimeline versions={offer.versions || []} />
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="esign" className="space-y-6">
            <Card className="bg-slate-900/50 backdrop-blur-sm border-purple-500/20">
              ...
              <CardContent>
                <EsignPanel signerPayload={offer.signer_payload} onPollStatus={() => {}} onResend={() => {}} />
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
