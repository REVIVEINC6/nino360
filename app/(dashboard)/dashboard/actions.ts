...
export async function getPersonalizedInsights(userId?: string): Promise<{ success: boolean; data?: any[]; error?: string }> {
  const supabase = await createServerClient()
...
export async function getPredictiveAnalytics(type: string = "finance", horizon: string = "90d"): Promise<{ success: boolean; data?: any[]; error?: string }> {
  const supabase = await createServerClient()
