...
// Safe AI helper: returns fallback when AI Gateway or provider isn't configured
const AI_WARN_FALLBACK = (process.env.AI_FALLBACK_WARN ?? "on").toLowerCase() !== "off"
async function safeGenerateJSON<T>(opts: { model: string; prompt: string }, fallback: T): Promise<T> {
  try {
    const { text } = await generateText({ model: opts.model as any, prompt: opts.prompt })
    try {
    ...
    if (AI_WARN_FALLBACK) {
      console.warn("[v0] AI generate fallback used:", (err as any)?.message || err)
    }
    return fallback
  }
}
...
export async function getKpis(): Promise<KPIs> {
  ...
  if (!user) {
    // If there's no authenticated user during SSR/dev, return safe empty KPIs
    return {
      ats: null,
      bench: null,
      finance: null,
...
export async function getAuditTimeline({ limit = 10 }: { limit?: number } = {}): Promise<AuditEntry[]> {
  ...
  if (error) {
    ...
    const isExpired = msg.includes("jwt expired") || msg.includes("token is expired") || msg.includes("expired")
    const LOG_AUDIT_WARN = (process.env.LOG_AUDIT_WARN ?? "on").toLowerCase() !== "off"
    // In dev or when JWT is expired, fall back silently (optionally warn)
    if (LOG_AUDIT_WARN && !isExpired) {
      console.error("[v0] Error fetching audit timeline:", (error as any)?.message || error)
    }
...
export async function getWeeklyDigest(): Promise<WeeklyDigest> {
  ...
  if (!user) {
    // If unauthenticated, return a safe empty weekly digest rather than throwing
    return {
      text: "Weekly digest not available",
      stats: {
        jobsOpened: 0,
        offersSent: 0,
        invoicesCreated: 0,
  ...
  const prompt = `Generate a brief weekly digest summary based on these stats:
- ${stats.jobsOpened} jobs opened
- ${stats.offersSent} offers sent
- ${stats.invoicesCreated} invoices created (${stats.invoicesPaid} paid)
- ${stats.employeesAdded} employees added
- ${stats.benchChanges} bench updates

Keep it under 50 words and professional.`

  try {
    const { text } = await generateText({ model: model as any, prompt, maxTokens: 100 })
    return { text, stats }
  } catch (error) {
...
export async function getSalesForecast(): Promise<ForecastData[]> {
  ...
  // Get forecast from ML predictions table
  const { data: forecast } = await supabase
    .from("ml_forecasts")
    .select("*")
    .eq("forecast_type", "sales")
    .eq("model_version", "latest")
    .order("forecast_date", { ascending: true })
    .limit(90)

  if (forecast && forecast.length > 0) {
    return forecast.map((row: any, i: number) => ({
      date: `Day ${i + 1}`,
      actual: row.actual_value || 0,
      forecast: row.predicted_value || 0,
      lower: row.lower_bound || 0,
      upper: row.upper_bound || 0,
    }))
  }

  // If no ML forecast available, return empty array
  return []
}
...
export async function getPersonalizedInsights(
  ...
  // Use AI to personalize insights based on user behavior (fallback to DB insights)
  const fallback = (insights as any) || []
...
export async function getPredictiveAnalytics(
  ...
  // Generate new predictions using AI (with safe fallback)
  const kpis = await getKpis()
  const digest = await getWeeklyDigest()

  const fallback = { predictions: [] as any[] }
  const predictions = await safeGenerateJSON<{ predictions: any[] }>(
    {
      model: "openai/gpt-4o-mini",
  ...
  // Best-effort cache only if we actually have predictions
  ...
  // Map to flat array for the widget if needed; here just return the predictions array
...
export async function detectAnomalies(): Promise<{ success: boolean; data?: any[]; error?: string }> {
  const supabase = await createServerClient()
  ...
  // Use AI to detect anomalies (safe fallback to empty list)
  const anomalies = await safeGenerateJSON<any[]>(
    {
      model: "openai/gpt-4o-mini",
  ...
  // Best-effort store anomalies as AI insights
  try {
    ...
    // ignore persistence issues in dev
  }
...
export async function getMLModelPerformance(): Promise<{
  ...
  try {
    ...
    if (error) {
      // In dev, the 'ml_models' table may not exist. Treat as empty data instead of error noise.
      if ((error as any)?.code === "PGRST205") {
        if (process.env.LOG_ML_PERF_WARN !== "off") {
          console.warn("[v0] ML model performance table missing; returning empty list")
        }
        return { success: true, data: [] }
      }
      if (process.env.LOG_ML_PERF_WARN !== "off") {
        console.warn("[v0] Error fetching ML model performance:", error)
      }
      return { success: false, error: (error as any)?.message || "Failed to fetch ML model performance" }
    }
    ...
    if (process.env.LOG_ML_PERF_WARN !== "off") {
      console.warn("[v0] Error in getMLModelPerformance:", error)
    }
