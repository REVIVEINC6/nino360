...
export async function getHotlistAnalyticsTiles() {
  const allowed = await hasPermission(PERMISSIONS.HOTLIST_DASHBOARD_VIEW)
  if (!allowed) {
    return {
      activeCampaigns: 0,
...
export async function getCampaignPerformance(campaignId?: string) {
  const allowed = await hasPermission(PERMISSIONS.HOTLIST_ANALYTICS_VIEW)
  if (!allowed) return []
  ...
  if (error) {
    // In dev, tables may be missing. Return empty list rather than throw.
    return []
  }
