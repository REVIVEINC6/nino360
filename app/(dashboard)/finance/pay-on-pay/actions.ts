...
export async function getPayOnPayContext() {
  ...
  // If no authenticated user, return nullable context. Callers will decide how to proceed.
  if (!user) {
    return { supabase, tenantId: null as any, userId: null as any }
  }
  ...
  // If user exists but no tenant mapping, return userId with null tenant
  if (!tenantMember?.tenant_id) {
    return { supabase, tenantId: null as any, userId: user.id }
  }
...
async function updateRunTotals(runId: string) {
  ...
  const totals = (items as Array<{ client_amount: any; vendor_amount: any; fee_amount: any; tax_amount: any }>).reduce(
    (acc: { totalClientReceipts: number; totalVendorPayouts: number; totalFees: number; totalTaxes: number }, item) => ({
...
export async function anchorSettlementRun(runId: string) {
  ...
  // Build Merkle tree
  const leaves = (items as Array<{ merkle_leaf_hash: string | null }>)
    .map((item) => item.merkle_leaf_hash as string)
    .filter(Boolean)
  // Ensure all leaves are hex; if any non-hex, hash the string
  const hexRe = /^[0-9a-fA-F]+$/
  const normalizedLeaves = (leaves as string[]).map((h: string) => (hexRe.test(h) ? h.toLowerCase() : cryptoHashHex(h)))
  const { merkleRoot } = await import("@/lib/crypto/merkle").then((m) => ({ merkleRoot: m.merkleRoot }))
  const root = merkleRoot(normalizedLeaves)
  ...
  anchor_root: root,
    ...
    await logAudit({
      ...
      entity: "settlement_run",
      entityId: runId,
  metadata: { tx_hash: txHash, merkle_root: root },
    })
...
function cryptoHashHex(s: string): string {
  // Use Node crypto to hash a string to hex
  // Defer require to avoid edge bundling issues in other contexts
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const nodeCrypto = require("crypto") as typeof import("crypto")
  return nodeCrypto.createHash("sha256").update(new TextEncoder().encode(s)).digest("hex")
}
...
export async function getPayOnPayAnalytics(dateFrom?: string, dateTo?: string) {
  try {
    ...
    if (!tenantId) return { success: true, data: { totalRuns: 0, completedRuns: 0, totalClientReceipts: 0, totalVendorPayouts: 0, totalFees: 0, totalTaxes: 0, netAmount: 0, anchoredRuns: 0, avgProcessingTime: 0 } }
  ...
  completedRuns: (runs as Array<any>).filter((r) => r.status === "completed").length,
  totalClientReceipts: (runs as Array<any>).reduce((sum: number, r: any) => sum + Number(r.total_client_receipts || 0), 0),
  totalVendorPayouts: (runs as Array<any>).reduce((sum: number, r: any) => sum + Number(r.total_vendor_payouts || 0), 0),
  totalFees: (runs as Array<any>).reduce((sum: number, r: any) => sum + Number(r.total_fees || 0), 0),
  totalTaxes: (runs as Array<any>).reduce((sum: number, r: any) => sum + Number(r.total_taxes || 0), 0),
  netAmount: (runs as Array<any>).reduce((sum: number, r: any) => sum + Number(r.net_amount || 0), 0),
  anchoredRuns: (runs as Array<any>).filter((r: any) => r.anchor_status === "anchored").length,
