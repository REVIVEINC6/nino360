...
async function getFinanceKPIs() {
  ...
  const outstandingAR = arData?.reduce((sum: number, inv: any) => sum + Number(inv.balance || 0), 0) || 0

  // Get AP balance (sum of all bill balances)
  const { data: apData } = await supabase.from("bills").select("balance").not("status", "in", '("void")')

  const outstandingAP = apData?.reduce((sum: number, bill: any) => sum + Number(bill.balance || 0), 0) || 0
