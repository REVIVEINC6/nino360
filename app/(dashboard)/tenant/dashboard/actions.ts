...
export async function getTenantContext() {
  try {
    ...
    const features = {
      copilot: featureFlags?.find((f: any) => f.feature_key === "copilot")?.enabled || false,
      audit_chain: featureFlags?.find((f: any) => f.feature_key === "audit_chain")?.enabled || false,
      analytics: featureFlags?.find((f: any) => f.feature_key === "analytics")?.enabled || false,
      crm: featureFlags?.find((f: any) => f.feature_key === "crm")?.enabled || false,
      talent: featureFlags?.find((f: any) => f.feature_key === "talent")?.enabled || false,
      hrms: featureFlags?.find((f: any) => f.feature_key === "hrms")?.enabled || false,
      finance: featureFlags?.find((f: any) => f.feature_key === "finance")?.enabled || false,
      bench: featureFlags?.find((f: any) => f.feature_key === "bench")?.enabled || false,
      vms: featureFlags?.find((f: any) => f.feature_key === "vms")?.enabled || false,
      projects: featureFlags?.find((f: any) => f.feature_key === "projects")?.enabled || false,
    }
...
export async function getAiDigest(dateRange: { from: string; to: string }) {
  ...
  const activeUsers = kpis?.filter((k: any) => k.status === "active").length || 0
...
Keep it professional, concise, and actionable. Do not include any PII or sensitive data.`
    ...
    // SDK types differ across environments; cast options to any for now to avoid strict type errors
    const genResult: any = await generateText({
  ...
  // usage shape may vary across SDKs; normalize safely via any and provide numeric defaults
  const usageAny = usage as any
  const totalTokens = typeof usageAny?.totalTokens === "number" ? usageAny.totalTokens : usageAny?.total_tokens ?? 0
  const promptTokens = typeof usageAny?.promptTokens === "number" ? usageAny.promptTokens : usageAny?.prompt_tokens ?? 0
  const completionTokens = typeof usageAny?.completionTokens === "number" ? usageAny.completionTokens : usageAny?.completion_tokens ?? 0

  const inputCost = (Number(promptTokens) / 1_000_000) * 0.15
  const outputCost = (Number(completionTokens) / 1_000_000) * 0.6
  const totalCost = inputCost + outputCost

  return {
    text: String(text || "").trim(),
    tokens: Number(totalTokens || 0),
...
function generateUserGrowthForecast(historicalData: any[]) {
  const userActions = historicalData.filter((entry: any) => String(entry.action).includes("user") || String(entry.action).includes("member"))
  ...
  try {
    ...
    const successRate =
      recentExecutions && recentExecutions.length > 0
        ? (recentExecutions.filter((e: any) => e.status === "completed").length / recentExecutions.length) * 100
        : 0
