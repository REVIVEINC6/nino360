...
const supa = () => {
  ...
  return createServerClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, {
    cookies: {
      get: (k) => c.get(k)?.value,
      set: (k, v, opts) => c.set(k, v, opts),
      remove: (k, opts) => c.set(k, "", { ...opts, maxAge: 0 }),
    },
  })
}
...
export async function listTenants(params?: {
  ...
  const { data: features } = await s
    .rpc("list_tenant_features", {
  ...
  const tenants: TenantListItem[] = memberships
    .map((m) => {
      ...
      const lastAudit = audits?.find((a) => a.tenant_id === m.tenant_id)
...
export async function quickLinks(tenantId: string) {
  ...
  const isAdmin = roleKeys.some((k) => k === "admin" || k === "super_admin" || k === "master_admin")
  const isManager = roleKeys.some((k) => k === "manager")
