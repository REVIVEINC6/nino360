...
export async function completeOnboarding(tenantId?: string) {
  // If tenantId isn't provided, resolve it from the tenant cookie/context
  if (!tenantId) {
...
// Client-friendly wrapper: update the current tenant's onboarding step using cookie-derived context
export async function updateOnboardingStep(current_step: number, _data: any) {
  const ctx = await getTenantContext()
  const tenantId = ctx.tenantId

  const completed = Array.isArray(ctx.completedSteps) ? ctx.completedSteps : []
  const completed_steps = completed.includes(current_step) ? completed : [...completed, current_step]

  // Delegate to the existing saveOnboardingProgress which validates the input
  return await saveOnboardingProgress({ tenant_id: tenantId, current_step, completed_steps })
}
...
export async function importCsv(input: unknown) {
  // Accept either base64 string or a File-like object from the client. Also allow tenant_id to be omitted
  // (resolve from cookie context) for convenience when called from client components.
  const schema = z.object({
    tenant_id: z.string().uuid().optional(),
    kind: z.enum(["contacts", "candidates"]),
    file: z.any(), // base64 string or File-like object
  })

  // If tenant_id not present, resolve from cookie/context
  let maybeInput = input as any
  if (!maybeInput || !maybeInput.tenant_id) {
    const ctx = await getTenantContext()
    maybeInput = { ...(maybeInput || {}), tenant_id: ctx.tenantId }
  }

  const body = schema.parse(maybeInput)
  const s = await supa()

  // Decode and parse CSV: support base64 string or File-like (has arrayBuffer)
  let csvContent: string
  if (typeof body.file === "string") {
    csvContent = Buffer.from(body.file, "base64").toString("utf-8")
  } else if (body.file && typeof body.file.arrayBuffer === "function") {
    const buf = Buffer.from(await body.file.arrayBuffer())
    csvContent = buf.toString("utf-8")
  } else {
    throw new Error("Unsupported file payload. Provide a base64 string or File object")
  }
