...
// Use the centralized server client factory which handles cookies and env validation
...
async function verifyAdmin() {
  ...
  // Allow a local/dev bypass of strict admin checks when explicitly enabled.
...
export async function listTenants(params?: unknown) {
  try {
    ...
    if (e?.message?.includes("Admin role required")) {
      return { rows: [], total: 0, error: "Unauthorized: Admin role required" }
    }
    throw e
  }
...
export async function listPlans() {
  ...
  try {
    ...
    if (error) {
      // If the plans table is not present in public schema, try core.plans or return empty
      if (error?.code === "PGRST205") {
        console.warn("[v0] plans table not found, trying core.plans")
        const { data: alt, error: altErr } = await supabase.from("core.plans").select("id, name, slug").order("name")
        if (altErr) {
          console.error("[v0] Error loading core.plans:", altErr)
          return []
        }
        return alt || []
      }
    ...
    console.error("[v0] Unexpected error loading plans:", err?.message || err)
    return []
  }
}
