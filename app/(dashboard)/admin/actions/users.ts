...
async function verifyAdmin() {
  ...
  const { data: userRoles, error } = await supabase.from("user_roles").select("role_id").eq("user_id", user.id)
...
export async function listUsers(params: unknown) {
  ...
  const usersWithDetails = await Promise.all(
    (data || []).map(async (user) => {
      ...
      return {
        ...user,
        roles: (roles || []).map((r) => r.label).join(", ") || "None",
        tenants: (tenants || []).map((t) => t.name).join(", ") || "None",
        groups:
          (groups || [])
...
export async function exportUserActivity(input: unknown) {
  ...
  let query = supabase.from("audit_logs").select("*").eq("user_id", user_id).order("created_at", { ascending: false })
...
export async function getUserDetails(input: unknown) {
  ...
  return {
    ...
    tenants: (tenants || []).map((t) => ({
      ...t,
      is_primary: userTenants?.find((ut) => ut.tenant_id === t.id)?.is_primary || false,
    })),
    activity: activity || [],
  }
}
