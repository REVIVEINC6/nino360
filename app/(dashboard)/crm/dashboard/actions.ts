...
export async function getKpis({ from, to }: { from: string; to: string }) {
  ...
  const pipeline = opportunities?.reduce((sum, opp) => sum + (opp.amount || 0), 0) || 0
  const prevPipeline = prevOpportunities?.reduce((sum, opp) => sum + (opp.amount || 0), 0) || 0

  const wonOpps = opportunities?.filter((o) => o.stage === "closed_won") || []
  const mtdRevenue = wonOpps.reduce((sum, opp) => sum + (opp.amount || 0), 0)
  const prevWonOpps = prevOpportunities?.filter((o) => o.stage === "closed_won") || []
  const prevRevenue = prevWonOpps.reduce((sum, opp) => sum + (opp.amount || 0), 0)

  const totalClosed = opportunities?.filter((o) => o.stage === "closed_won" || o.stage === "closed_lost").length || 0
  const winRate = totalClosed > 0 ? (wonOpps.length / totalClosed) * 100 : 0
  const prevTotalClosed =
    prevOpportunities?.filter((o) => o.stage === "closed_won" || o.stage === "closed_lost").length || 0
...
export async function getPipelineHealth({ from, to }: { from: string; to: string }) {
  ...
  const byStage =
    opportunities?.reduce(
      (acc, opp) => {
        const existing = acc.find((s) => s.stage === opp.stage)
        if (existing) {
          existing.count++
  ...
  const aging =
    opportunities?.reduce(
      (acc, opp) => {
        ...
        const existing = acc.find((a) => a.bucket === bucket)
        if (existing) {
          existing.count++
          existing.amount += opp.amount || 0
        } else {
...
export async function getForecast({ horizonDays = 90 }: { horizonDays?: number }) {
  ...
  for (let i = 0; i <= horizonDays; i += 7) {
    ...
    const oppsClosingByDate = opportunities?.filter((o) => new Date(o.close_date) <= date) || []
    ...
    const best = oppsClosingByDate
      .filter((o) => (o.probability || 0) >= 50)
      .reduce((sum, o) => sum + (o.amount || 0) * ((o.probability || 0) / 100), 0)

    const pipeline = oppsClosingByDate.reduce((sum, o) => sum + (o.amount || 0), 0)
...
export async function getAtRisk({ limit = 10 }: { limit?: number }) {
  ...
  const atRisk =
    opportunities
      ...
      .filter((o) => o.days_stale > 7 || o.reason === "pushout" || o.reason === "low-prob")
...
export async function verifyHash(hash: string) {
  ...
  const isValid = verifyHashChain(record.prev_hash || "", prevRecord?.hash || "", JSON.stringify(record))
...
Pipeline by Stage:
${pipeline.byStage.map((s) => `- ${s.stage}: ${s.count} deals, $${(s.amount / 1000).toFixed(1)}K`).join("\n")}
