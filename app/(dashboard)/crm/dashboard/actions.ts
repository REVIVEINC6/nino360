...
export async function getKpis({ from, to }: { from: string; to: string }) {
  ...
  const totalClosed =
    opportunities?.filter((o: any) => o.stage === "closed_won" || o.stage === "closed_lost").length || 0
  ...
  return (
    (await Promise.all(
      (opportunities || []).map(async (o: any) => {
        ...
        const masked = await applyFieldPermissions(
          provider,
          (supabase.auth as any)?.user?.id || "",
          tenantId,
          "crm_opportunities",
          o,
        )
        return {
          id: masked.id || o.id,
          name: masked.name || o.name,
          stage: masked.stage || o.stage,
          amount: masked.amount || o.amount || 0,
          probability: masked.probability || o.probability || 0,
          close_date: masked.close_date || o.close_date,
          owner: (masked as any).profiles?.full_name || o.profiles?.full_name || "Unassigned",
          avatar: (masked as any).profiles?.avatar_url || o.profiles?.avatar_url,
        }
      }),
    )) || []
  )
...
export async function getRPAWorkflows() {
  ...
  if (error) {
    console.error("[v0] Error fetching RPA workflows:", error)
    return { success: false, data: [] }
  }

  // Get execution stats
  const { data: executions } = await supabase
    .from("rpa_workflow_executions")
    .select("workflow_id, status, created_at")
    .in("workflow_id", workflows?.map((w) => w.id) || [])
    .gte("created_at", new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString())

  const emailFollowups =
    executions?.filter((e) => workflows?.find((w) => w.id === e.workflow_id && w.name?.includes("Email"))).length || 0

  const leadEnrichment =
    executions?.filter((e) => workflows?.find((w) => w.id === e.workflow_id && w.name?.includes("Lead"))).length || 0
...
export async function getAIInsights({ kpis, pipeline }: { kpis: any; pipeline: any }) {
  const { features } = await getContext()
  if (!features.copilot) {
  ...
  // Fetch recent opportunities for analysis
  const { data: opportunities } = await supabase
    .from("crm_opportunities")
    .select("*")
    .eq("tenant_id", tenantId)
  ...
  const context = `
Analyze this CRM data and provide actionable insights:

KPIs:
- Pipeline: $${(kpis.pipeline / 1000).toFixed(1)}K (${kpis.deltas.pipeline > 0 ? "+" : ""}${kpis.deltas.pipeline.toFixed(1)}%)
- MTD Revenue: $${(kpis.mtdRevenue / 1000).toFixed(1)}K
- Win Rate: ${kpis.winRate.toFixed(1)}%
- Avg Sales Cycle: ${kpis.avgCycleDays} days

Pipeline Health:
${pipeline?.byStage?.map((s: any) => `- ${s.stage}: ${s.count} deals, $${(s.amount / 1000).toFixed(1)}K`).join("\n")}

Recent Activity: ${activities?.length || 0} activities in last 24h
Open Opportunities: ${opportunities?.filter((o: any) => !o.stage.includes("closed")).length || 0}

Provide 4 specific insights covering:
1. An opportunity (high-value deals or trends)
2. A risk (pipeline issues or bottlenecks)
3. A trend (positive momentum or patterns)
4. A recommendation (actionable next step)

Format each insight as JSON with: type, title, description, impact (high/medium/low), confidence (0-100), actionable (boolean)
`
  ...
  const insights: any[] = []
    ...
    await supabase.from("app_audit_log").insert({
      tenant_id: tenantId,
      action: "crm.dashboard:ai_insights",
      entity_type: "ai_usage",
      metadata: {
        tokens: usage?.totalTokens || 0,
        model: "gpt-4o-mini",
    ...
    return { success: true, insights, tokens: usage?.totalTokens || 0 }
  } catch (error) {
...
export async function getBlockchainAuditTrail() {
  ...
  const verifiedCount = auditRecords?.filter((r) => r.hash).length || 0
  const lastSync = auditRecords?.[0]?.created_at
    ? formatDistanceToNow(new Date(auditRecords[0].created_at), { addSuffix: true })
    : "Never"
  const latestHash = auditRecords?.[0]?.hash || ""
