...
export async function getEngagementAnalytics() {
  ...
  const totalSent =
    (sequences?.reduce((sum, s) => sum + (s.total_sent || 0), 0) || 0) +
    (campaigns?.reduce((sum, c) => sum + (c.sent_count || 0), 0) || 0)

  const totalOpened =
    (sequences?.reduce((sum, s) => sum + (s.total_opened || 0), 0) || 0) +
    (campaigns?.reduce((sum, c) => sum + (c.opened_count || 0), 0) || 0)

  const totalClicked =
    (sequences?.reduce((sum, s) => sum + (s.total_clicked || 0), 0) || 0) +
    (campaigns?.reduce((sum, c) => sum + (c.clicked_count || 0), 0) || 0)

  const totalReplied = sequences?.reduce((sum, s) => sum + (s.total_replied || 0), 0) || 0
