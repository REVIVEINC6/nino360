...
export async function getSequences() {
  ...
  // If there's no user (SSR or unauthenticated), return an empty list so pages can render
  if (!user) return []
...
export async function getEngagementAnalytics() {
  ...
  if (!user) {
    // During server-side rendering a user may not be available (dev or unauthenticated).
    // Return safe default analytics so pages can render without throwing.
    return {
      totalSent: 0,
      totalOpened: 0,
      totalClicked: 0,
