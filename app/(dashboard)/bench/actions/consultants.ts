...
export async function listConsultants(filters?: {
  ...
  if (error) {
    ...
    // In development environments, many schemas/tables may be missing. Return empty list rather than throw.
    return []
  }
...
export async function getDashboardKPIs() {
  ...
  // Count by status
  const onBench = consultants?.filter((c: any) => c.status === "bench").length || 0

  // Calculate average bench days
  const benchConsultants = consultants?.filter((c: any) => c.status === "bench" && c.rolloff_date) || []
  const avgBenchDays =
    benchConsultants.length > 0
      ? Math.round(
          benchConsultants.reduce((sum: number, c: any) => {
...
export async function listSources() {
  ...
  const { data, error } = await supabase.from("bench.sources").select("name").order("name")
