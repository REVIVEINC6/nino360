...
import { Building2, FileCheck, Send, DollarSign } from "lucide-react"
import { TwoPane } from "@/components/layout/two-pane"
import { VMSSidebar } from "@/components/vms/vms-sidebar"

export default function VMSDashboardPage() {
  return (
  <TwoPane right={<VMSSidebar />}>
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between border-b pb-2">
                  <div>
                    <p className="font-medium">John Doe - Senior Developer</p>
                    <p className="text-sm text-muted-foreground">TechVendor Inc.</p>
                  </div>
                  <span className="text-sm text-blue-600">Shortlisted</span>
                </div>
              ))}
            </div>
          </CardContent>
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center justify-between border-b pb-2">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">GlobalStaff LLC</p>
                  </div>
